...
void priority_based_scheduler(struct cpu *c, int queue , int reverse)
{
  struct proc *p = 0;
  struct proc *p1 = 0;
...
void round_robin_scheduler(struct cpu *c, int queue)
{
  struct proc *p;
  // Enable interrupts on this processor.
  sti();
...
void scheduler(void)
{
  ...
  for (;;)
  {
    ...
    {
      priority_based_scheduler(c, 1 , 0);
    }
  }
}
...
{
  return myproc()->systemcall_number[pid];
}
